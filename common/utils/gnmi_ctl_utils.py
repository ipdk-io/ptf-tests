...
def gnmi_get_params_verify(params):
    ...
    for param in params:
        ...
        for entry in param.split(',')[2:]:
            if port_config.GNMICTL.gnmi_ctl_get(mandatory_param, entry.split(':')[0]) != entry.split(':')[1]:
                passed=False
...
def gnmi_get_params_counter(param):
    ...
    value = port_config.GNMICTL.gnmi_ctl_get_counter(mandatory_param,"counters").strip()
